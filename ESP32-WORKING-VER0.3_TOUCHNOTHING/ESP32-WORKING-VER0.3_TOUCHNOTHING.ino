// --- CÓDIGO FINAL PARA TORRETA NERF (V2.15 - DEPURACIÓN VERBOSA) ---
...
const float CONFIDENCE_THRESHOLD = 0.80;
const int CENTER_DEAD_ZONE = 8;
...
// --- AJUSTES DE DISPARO ---
const int MIN_REV_TIME = 5000;
const int FIRE_COOLDOWN = 1000;

// --- CONFIGURACIÓN DE DEPURACIÓN ---
// Aumenta este valor (en ms) para pausar el código después de una orden de movimiento.
...
void setup() {
  ...
  Serial.println("--- INICIANDO TORRETA V2.15 (Depuración Verbosa) ---");
  ...
  panStepper.runToNewPosition(0); // Vuelve al centro
...
void parseAndTrack(String data) {
  ...
  if (abs(error) > CENTER_DEAD_ZONE) {
    ...
    if (error < 0) { // Mover a la izquierda
      Serial.println("[DEBUG] Dirección: IZQUIERDA.");
      newPos -= PAN_STEPS_TO_MOVE;
    } else { // Mover a la derecha
      Serial.println("[DEBUG] Dirección: DERECHA.");
      newPos += PAN_STEPS_TO_MOVE;
    }

    // Limita la nueva posición
    if (newPos < -MAX_PAN_STEPS_FROM_CENTER) {
      newPos = -MAX_PAN_STEPS_FROM_CENTER;
      Serial.println("[DEBUG] INFO: Límite izquierdo alcanzado.");
    }
    if (newPos > MAX_PAN_STEPS_FROM_CENTER) {
      newPos = MAX_PAN_STEPS_FROM_CENTER;
      Serial.println("[DEBUG] INFO: Límite derecho alcanzado.");
    }
